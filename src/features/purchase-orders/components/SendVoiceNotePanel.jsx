...
/**
 * Standalone "send a voice / text note" panel for the PO detail page.
 *
 * Mounted on the vendor portal view (vendor talking to procurement) and the
 * admin view (admin can post on behalf of vendor). Picked up by the
 * VoiceNotesPanel above — the new entry is appended to approval_history
 * server-side, refetched via the updated PO object the endpoint returns.
 *
 * Required: either typed text or a voice clip. Empty submit blocked
 * client-side AND server-side.
 */
export default function SendVoiceNotePanel({ poNumber, onSent }) {
  const [comment, setComment] = useState("");
  const [voiceNote, setVoiceNote] = useState(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState(null);
  const vendorNote = usePOStore((s) => s.vendorNote);
  ...
  const send = async () => {
    ...
    setError(null);
    try {
      const updated = await vendorNote(poNumber, {
    ...
    <section className="glass-card rounded-2xl overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        <p className="text-xs text-text-muted">
          Speak or type a quick message — useful if you'd rather talk than
          write, or want to share an update without sending a document.
          Procurement, HOD, CFO and CEO will see and hear it.
        </p>
        ...
        <textarea
          ...
          disabled={busy}
          placeholder="e.g. Delivery delayed by one day — fresh ETA tomorrow noon."
